...
class StorageGUI:
    ...
    def setup_ui(self):
        ...
        tk.Label(legend_frame, text="  Online  ", bg="lightgreen", relief=tk.RAISED).pack(side=tk.LEFT, padx=2)
        tk.Label(legend_frame, text="  Offline  ", bg="red", fg="white", relief=tk.RAISED).pack(side=tk.LEFT, padx=2)
        tk.Label(legend_frame, text="  Local Parity  ", bg="mediumpurple", fg="white", relief=tk.RAISED).pack(side=tk.LEFT, padx=2)
        tk.Label(legend_frame, text="  Global Parity  ", bg="dodgerblue", fg="white", relief=tk.RAISED).pack(side=tk.LEFT, padx=2)
    ...
    def create_dnode_layout(self):
        ...
        for dnode_id, dnode in enumerate(self.storage.dnodes):
            ...
            for idx, drive_id in enumerate(dnode['all_drives']):
                ...
                # Determine drive color scheme
                drive_type = self.storage.get_drive_type(drive_id)
                
                # Create button
                btn = tk.Button(drives_grid, text=f"{drive_id}\n0000", 
                               width=6, height=2,
                               bg="white",
                               font=("Courier", 7),
                               command=lambda x=drive_id: self.toggle_drive(x))
                btn.grid(row=grid_row, column=grid_col, padx=1, pady=1, sticky="nsew")
    ...
    def update_drive_display(self, drive_id):
        ...
        # Update color based on type and status
        if not online:
            btn.config(bg="red", fg="white")
        elif drive_type == "Local Parity":
            btn.config(bg="mediumpurple", fg="white")
